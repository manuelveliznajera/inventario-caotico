        ...
        success: function(data) {
            if (data.success == true) {
               ...
               $("#txtBuscapedido").keypress(function(e) {
                ...
                if(code==13){
                codigoBarra=parseInt($( "#txtBuscapedido" ).val());
                enviarajax(codigoBarra);
                limpiar();
                }
            });
                limpiar();   
...
function limpiar(){
    ...
    $( "#txtBuscapedido" ).focus();
      ...
      $(document).off("click",".btnexistencias");
    $(document).on("click", ".btnexistencias", function(e){
        var codigoBarra = 0;
        console.log("le dio click a btn existencias");
            ...
            success: function(data) {
                if (data.success == true) {
                  ...
                  */  
                    ...
                    alertify.error("Error fuera php40");
    ...
    $(document).on("click",".btncargaregresos", function(e) {
        ...
        /*if (xtipoop=='E0'){
            xpaginaconsulta='22'; // proceso que muestra grupos de despacho
        }
        else{
            xpaginaconsulta='14'; // encabezados de piking
        }*/
        //websocket(xtipoop);
        $.ajax({ async:true,
            url: 'controlador.php?action='+xpaginaconsulta,
            type: 'post',
            data: {
                'tipoop': xtipoop,
                'tipomov': xtipomov,
    ...
    /*
    $(document).off("click",".btncargartarea_egreso");
    $(document).on("click",".btncargartarea_egreso", function(e) {
        //if (corriendo2==0) {lectura_tecladof2();}
        cunico = $(this).attr("id");
        pedido= $(this).attr("pedido");
        //console.log(cunico);
        clasempresa = $(this).attr("empresa");
        //console.log("Funciónes en proceso de desarrollo.");
        
        $.ajax({ async:true,
            url: 'controlador.php?action=33', // detalle de ordenes pendientes de packing
            type: 'post',
            data: {
                'cunico': cunico,
    ...
    */
    // cargar encabezados de egreso con grupo
    ...
    $(document).on("click","#btn_finalizargrupo", function(e) {
        ...
        alertify.alert().setting({'title':'Egresos de bodega',
            ...
            'message': 'Al completar el grupo, este dejara de ser visible para su usuario.¿Desea continuar? ', 
            'defaultFocus': 'ok' , 
            'onok': function()
                { 
                cuadromodalabierto= 0;
                 ...
                 if (data.success == true) {
                   // alertify.success(data.msj);
                    //console.log(data.grupo);
                     $contenedor.prepend($modelo);//es para ir creando un div de cada grupo creado si todo funciona bien
                     setTimeout(function() 
                      {
                        $(".btncargartarea_nuevogrupo").trigger("click") ;// acciona el evento click para la linea 480
                      }, 500)
    ...
    $(document).on("click",".btncargartarea_seleccionagrupo", function(e) {
        ...
        /*
        $.ajax({ async:true,
            url: 'controlador.php?action=33', // selecciona pedido
            type: 'post',
            data: {'grupopicking': $idagrupa},
            dataType: 'json',
            success: function(data) {
                $("#contenidopg").html(data.detalle);
               if (data.success == true) { */
